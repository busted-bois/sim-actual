...
`blueline-2` and `ks_vision+blueline`. Ordered high-level first; the later parts
are line-by-line.
...
`detect_track()` — exists **only** on `ks_vision+blueline`. It was recovered in
`7140770` from the discarded `useless_files` work.
...
| `tests/test_blue_line_pilot.py` (60+) | wire-sign direction for every axis, corner braking and the rise cap, 25 km/h cap, the search ladder, gate-bias engage/skip cases, confidence fade, band-gated heading, `dcx` rate damping and its cap, `gz` damping sign, gate commit arm/hold/expire/disarm, vision staleness, and the whole race-gating state machine |

Both are in `make test`. Note that **CI runs only ruff and doc checks** — unit
tests are not gated, so a committed baseline can carry failing tests.
