...
class GateTracker:
    ...
    def try_seed_from_track(self) -> None:
        if self._seeded_from_track or self.kf.initialized:
            return
    ...
    def _drone_pose(self) -> tuple[np.ndarray, np.ndarray] | None:
        # Same local NED frame as track_gates / pilot odometry — not VIO.
    ...
    def process_detection(
        ...
        self.kf.update(meas["pos_ned"], meas["quat"], sigma_pos, sigma_att)
        self._updates += 1
        self._publish(
            detected=True,
            quality=quality,
