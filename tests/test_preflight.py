...
class PreflightRaceGoTests(unittest.TestCase):
    ...
    def test_vision_ready(self):
        self.assertFalse(vision_ready({}))
        self.assertTrue(vision_ready({"camera": {"received_at": 1.0}}))

    def test_imu_ready(self):
        self.assertFalse(imu_ready({}))
        self.assertTrue(imu_ready({"imu": {"ax": 0.0}}))

    def test_connect_ready_imu_or_vision(self):
        self.assertFalse(connect_ready({}))
        self.assertTrue(connect_ready({"imu": {}}))
        self.assertTrue(connect_ready({"camera": {"received_at": 1.0}}))

    def test_wait_for_connect_succeeds_with_imu(self):
        data = {"imu": {"pressure_alt": 100.0}}
        self.assertTrue(wait_for_connect(data, timeout_s=0.5))

    def test_wait_for_session_ready_succeeds(self):
        data = {"camera": {"received_at": 1.0}}
        self.assertTrue(wait_for_session_ready(data, timeout_s=0.5))

    def test_wait_for_session_ready_cancelled(self):
        with patch("simulator.preflight.PREFLIGHT_POLL_S", 0.01):
            cancel = _MockCancel(cancelled=True)
            self.assertFalse(wait_for_session_ready({}, timeout_s=1.0, cancel=cancel))

    def test_wait_for_race_go_cancelled(self):
        data = {
            "race_status": {
        ...
        with patch("simulator.preflight.RACE_GO_POLL_S", 0.01):
            cancel = _MockCancel(cancelled=True)
            self.assertFalse(wait_for_race_go(data, timeout_s=1.0, cancel=cancel))

    def test_wait_for_race_status_succeeds(self):
        data = {"race_status": {"sim_boot_time_ms": 0}}
        self.assertTrue(wait_for_race_status(data, timeout_s=0.5))

    def test_wait_for_vq2_ready_succeeds(self):
        data = {
            "race_status": {"race_start_boot_time_ms": -1},
            "camera": {"received_at": 1.0},
        }
        self.assertTrue(wait_for_vq2_ready(data, timeout_s=0.5))

    def test_wait_for_vq2_ready_no_track_gates_required(self):
        data = {"race_status": {"sim_boot_time_ms": 0}}
        with patch("simulator.preflight.PREFLIGHT_POLL_S", 0.01):
            self.assertFalse(wait_for_vq2_ready(data, timeout_s=0.05))
