...
class LoadCalibrationTests(unittest.TestCase):
    ...
    def test_roundtrip_via_flightlab_save(self):
        # The harness writes (flightlab.metrics.save_calibration), rl reads.
        from flightlab import metrics

        with tempfile.TemporaryDirectory() as d:
            path = os.path.join(d, "calibration.json")
            with (
                mock.patch.object(metrics, "CAL_PATH", path),
                mock.patch.object(calibration, "CAL_PATH", path),
            ):
                metrics.save_calibration({"hover_thrust": 0.27, "rate_tau_s": 0.06})
...
class ConsumerDefaultTests(unittest.TestCase):
    ...
    def test_defaults_without_calibration(self):
        ...
        self.assertEqual(spec.HOVER_THRUST, 0.5)
        self.assertAlmostEqual(env.THRUST_ACCEL, spec.GRAVITY / 0.5, places=9)
