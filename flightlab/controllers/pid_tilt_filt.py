"""PID + tilt + slew-limit + 1st-order low-pass on thrust (τ ≈ 50 ms)."""
...
I_LIMIT = 0.08
TAU_S = 0.050
SLEW_PER_S = 2.0  # thrust units per second


class PidTiltFiltController:
    name = "pid_tilt_filt"
    ...
    def __init__(
        ...
        self.slew = slew
        self._i = 0.0
        self._thrust_f = hover

    def reset(self, s: State) -> None:
        self._i = 0.0
        self._thrust_f = self.hover
    ...
    def update(self, s: State, target: Target, dt: float) -> Cmd:
        ...
        desired = (u_pd + self._i) * tilt_comp_factor(s.roll, s.pitch)
        desired = float(max(THRUST_MIN, min(THRUST_MAX, desired)))

        # Slew limit
        dt = max(dt, 1e-4)
        max_step = self.slew * dt
        delta = max(-max_step, min(max_step, desired - self._thrust_f))
        slewed = self._thrust_f + delta

        # 1st-order low-pass
        alpha = dt / (self.tau + dt)
        self._thrust_f = self._thrust_f + alpha * (slewed - self._thrust_f)
        self._thrust_f = float(max(THRUST_MIN, min(THRUST_MAX, self._thrust_f)))
        ...
        return Cmd(rr, pr, 0.0, self._thrust_f)
